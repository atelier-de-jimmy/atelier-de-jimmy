import { infosCitroenJimmy } from '@/constants';
  ...
  return (
    <section className="container section flex-col-center gap-8">
      <h2 className="sub-title">
        Pourquoi choisir l’Atelier de Jimmy <br className="hidden lg:block" />{' '}
        pour votre Citroën à Bergerac
      </h2>
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl sm:mt-10">
        {infosCitroenJimmy.map(({ emoji, title, text }, index) => (
          <li
            key={index}
